...
const sina_hk_js_decode = `function d(t) {
...
func init() {
	_, err := vm.RunString(sina_hk_js_decode)
	if err != nil {
		panic(err)
	}
	js_func_d = vm.Get("d")
...
func SinaJsDecodeByAssertFunction(text string) (interface{}, error) {
	jsDecode, ok := js.AssertFunction(js_func_d)
	if !ok {
	...
	script, err := jsDecode(js.Undefined(), vm.ToValue(text))
	fmt.Print(script)
	return script.Export(), err
}

func SinaJsDecode(text string) (interface{}, error) {
	var jsDecode func(string) []interface{}
	err1 := vm.ExportTo(js_func_d, &jsDecode)
	if err1 != nil {
		panic(err1)
	}
	tmp := strings.Split(text, "=")
